...
- 大多数普通电商商品：达人带货 4 条、剧情广告 2 条、品牌大片 2 条。
- 食品零食：必须包含 `食品口感连击种草（XQ-05）`，可加 `场景错位短剧反转（XQ-01）`。
- 礼品 / 套装：必须包含 `礼品心意开箱（XQ-06）` 或 `套装清单展示（XQ-07）`。
- AI 工具 / SaaS：必须包含 `AI 工具能力展示（XQ-02）`。
- 智能家居小物：必须包含 `智能小物场景连发（XQ-08）`。
- 保健食品 / 功能型食品：可用 `成分背书促销（XQ-09）`，但价格、成分、产地、认证必须来自用户输入。
